...
function action3(state: State): string[] {
  ...
  const closestToBase = minBy(aggressiveMonsters, (m) =>
    distanceSquared(m.position, state.basePosition)
  );
  return state.selfHeroes.map((hero, i) => {
    ...
    if (closestMonster !== undefined) {
      ...
      return `MOVE ${state.basePosition.x + dx} ${state.basePosition.y +
        dy} ಠ_ಠ`;
    }
  });
}
...
function goalie(hero: Hero, state: State): string {
  ...
  const closestMonster = minBy(state.monsters, (m) =>
    distanceSquared(targetPosition, m.position)
  );
  if (closestMonster !== undefined) {
    ...
    if (distance(monsterFuturePosition, targetPosition) < windDistance / 4) {
      return `SPELL WIND ${closestMonster.position.x} ${closestMonster.position.y} FUS`;
    }
    return `MOVE ${monsterFuturePosition.x} ${monsterFuturePosition.y} GRRR`;
  }
...
export function run() {
  ...
  while (true) {
    ...
    const targetingMonsters = state.monsters
      .filter((m) => m.isTargetingBase)
      .map(({ id, health, position }) => ({
        id,
        health,
        position,
        distance: distance(position, state.basePosition),
      }));
    sortBy(targetingMonsters, (tm) => tm.distance).forEach((m) =>
      console.error({ ...m, wind: m.distance < 1280 })
    );
